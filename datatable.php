...
if($_GET['type']=="feesearch")
{
	...
	while ( $aRow = $rResult->fetch_assoc()  )
	{
		$feesText = '';
		if ($aRow['classpaid'] > 0) {
			$feesText .= 'Semester Fees: ' . $aRow['classpaid'] . '<br>';
		}
		
		if ($aRow['buspaid'] > 0) {
			$feesText .= 'Bus Fees: ' . $aRow['buspaid'] . '<br>';
		}
		
		if ($aRow['vanpaid'] > 0) {
			$feesText .= 'Van Fees: ' . $aRow['vanpaid'] . '<br>';
		}
		
		if ($aRow['skillpaid'] > 0) {
			$feesText .= 'Skill Fees: ' . $aRow['skillpaid'] . '<br>';
		}
		
		if ($aRow['otherpaid'] > 0) {
			$feesText .= 'Other Fees: ' . $aRow['otherpaid'] . '<br>';
		}
		
		$row = array(
                    html_entity_decode($aRow['sname'].'<br/>'.$aRow['roll_no']),
                    $aRow['grade'].' '.$aRow['yearofstart'].'-'.$aRow['yearofend'],
					$aRow['sem'],
                    $aRow['fees'],
					$feesText,
					$aRow['balance'],
